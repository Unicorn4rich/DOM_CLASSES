...
btn2?.addEventListener("click", (e)=>{ // is addEvent ki sari property`s (e) parameter mein aa gai.
...
myInput?.addEventListener('change', (e)=>{  // input ke andar kuch likh kar enter karenge to ye us input wali value ko dosri value se compare krwa ke dega. change Event se 2 values ko compare karwa sakty hain.
...
function myfunc(){  // HTML button Element par jese click karenge uske andar ka onclick ka attribute chalega jiske andar myfunc() ko chlany ke liye calling line di hui hai jese hi wo calling line chalegi typscript mein moujod 
